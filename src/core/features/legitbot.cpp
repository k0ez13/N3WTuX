...
void Features::LegitBot::createMove(CUserCmd* cmd) {
    static bool flickedAlready = false;
    if ((Menu::CustomWidgets::isKeyDown(CONFIGINT("Legit>LegitBot>Default>Key")) || CONFIGBOOL("Legit>LegitBot>Default>Always on")) || (Menu::CustomWidgets::isKeyDown(CONFIGINT("Legit>Triggerbot>Key")) && CONFIGBOOL("Legit>Triggerbot>Magnetic")) && Interfaces::engine->IsInGame() && Globals::localPlayer && Globals::localPlayer->health() > 0) {
        ...
        if (weapon) {
            ...
            bool snap = CONFIGBOOL("Legit>LegitBot>Default>Snap");
            ...
            if ((std::find(std::begin(pistols), std::end(pistols), weapon->itemIndex() & 0xFFF) != std::end(pistols)) && CONFIGBOOL("Legit>LegitBot>Pistol>Override")) {
                ...
                snap = CONFIGBOOL("Legit>LegitBot>Pistol>Snap");
            }
                ...
                snap = CONFIGBOOL("Legit>LegitBot>Heavy Pistol>Snap");
            }
                ...
                snap = CONFIGBOOL("Legit>LegitBot>Rifle>Snap");
            }
                ...
                snap = CONFIGBOOL("Legit>LegitBot>SMG>Snap");
            }
                ...
                snap = CONFIGBOOL("Legit>LegitBot>Scout>Snap");
            }
                ...
                snap = CONFIGBOOL("Legit>LegitBot>AWP>Snap");
            }
                ...
                snap = CONFIGBOOL("Legit>LegitBot>Heavy>Snap");
            }
